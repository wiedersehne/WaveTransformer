# Modified from https://github.com/square/pysurvival/blob/master/pysurvival/models/simulations.py
...
class SimulationModel(BaseModel):
    ...
    * risk_parameter: double (default = 1.)
    ...
    def random_data(N, key):
        ...
        distributions = {
            'normalised_age': np.random.normal(loc=0.0, scale=1.0, size=N),
            'gender': scipy.stats.bernoulli.rvs(p=0.5, size=N),
            'CNA_position': np.random.uniform(low=0.0, high=1.0, size=N),
    ...
    def generate_data(self,
        ...
        * dataset: pandas.DataFrame
        ...
        dataset.head()
        """
        num_features = len(feature_distributions)
        ...
        columns = []
        for i, _key in enumerate(feature_distributions):
            key, X[:, i] = self.random_data(num_samples, _key)
...
if __name__ == "__main__":
    ...
    for cancer in range(5):
        # Initializing our modified simulation model
        sim = SimulationModel(survival_distribution='gompertz',
                              risk_type='linear',
                              censored_parameter=5.0,
                              alpha=0.1,
        ...
        # Generating N Random samples
        N = 1000
        feature_distributions = ["normalised_age", "gender"] + ["CNA_position"] * 10
        print(feature_distributions)
        dataset = sim.generate_data(feature_distributions, num_samples=N)

        print(dataset)
        display_baseline_simulations(sim, figure_size=(10, 3))
