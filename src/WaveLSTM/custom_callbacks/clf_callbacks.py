...
class ClfMultiScaleEmbedding(Callback, BaseCallback):
    ...
    def run_callback(self, features, labels, log_name, _trainer, _pl_module, stack=False):
        ...
        for lbl in np.unique(labels):
            ...
            biclustering = SpectralBiclustering(n_clusters=(attention.shape[1], 1), method="log", random_state=0)
            ...
            # attention_new[lbl_idx, :]  = atn_cls[:, np.argsort(biclustering.column_labels_)]
            # features_new[lbl_idx, :]  = atn_cls[:, np.argsort(biclustering.column_labels_)]
