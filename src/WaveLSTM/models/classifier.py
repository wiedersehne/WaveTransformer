...
class AttentiveClassifier(pl.LightningModule, ABC):
    ...
    def __init__(self,
        ...
        # Classifier network
        self.clf_net = nn.Sequential(nn.Dropout(config['dropout']),
    ...
    def forward(self, x: torch.tensor):
        ...
        output, meta_results = self.self_attentive_encoder(x)
        meta_results.update({"M": output})
    ...
    def loss_function(self, targets, pred_t, meta) -> dict:
        ...
        eye = torch.stack([torch.eye(self.self_attentive_encoder.attention_hops) for _ in range(bsz)], dim=0)
        p = torch.norm(torch.bmm(atn, atn_t) - eye.to(device))
        penalty = self.self_attentive_encoder.attention_pen * p
...
def create_classifier(classes, seq_length, strands, chromosomes,
    ...
    config = {"dropout": 0.0,
              ...
              "nfc": 64,                          # hidden layer size for MLP in the classifier
    ...
    _model = AttentiveClassifier(encoder_config=encoder_config, config=config)
    print(_model)
