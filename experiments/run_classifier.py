...
def experiment(simulated=False):
    ...
    if simulated:
        ...
        dm = TCGA.data_modules.ascat.ASCATDataModule(cancer_types=cancer_types, batch_size=128) # , custom_edges=edges2wavelet)

    print(dm)
    # latent_dimension = 10 if not simulated else stacked_bases.shape[0]
    seq_length = 1000 if not simulated else 20
    chromosomes = 23 if not simulated else 1

    # Create encoder
    encoder = SequenceEncoder(in_features=seq_length*chromosomes, out_features=n_classes, n_hidden=128, n_layers=3,
